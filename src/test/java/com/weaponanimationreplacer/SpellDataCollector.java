...
public class SpellDataCollector
{
	...
	private void spellStart(Projectile projectile)
	{
		...
		currentSpellChebyshevDistance = plugin.chebyshevDistance(client.getLocalPlayer(), plugin.lastCastedOnActorToTransmogHitSplashOn, false);
		...
		System.out.println("\tspelldata: " +
			"chebyshev: " + plugin.chebyshevDistance(client.getLocalPlayer(), plugin.lastCastedOnActorToTransmogHitSplashOn, false) + " (" + plugin.chebyshevDistance(client.getLocalPlayer(), plugin.lastCastedOnActorToTransmogHitSplashOn, true) + ") " +
	...
	{
		if (plugin.lastCastedOnActorToTransmogHitSplashOn != null)
		{
			int spotAnimFrame = plugin.lastCastedOnActorToTransmogHitSplashOn.getSpotAnimFrame();
			if (spotAnimFrame == 1 && lasttargetspotanimframe == 0)
			{
				...
				currentProjectile.hitGfx = plugin.lastCastedOnActorToTransmogHitSplashOn.getGraphic();
				currentProjectile.hitGfxHeight = plugin.lastCastedOnActorToTransmogHitSplashOn.getGraphicHeight();
...
//				currentProjectile.castGfxStart = client.getGameCycle() - currentProjectile.spellStart;
		...
		} else if (graphicChanged.getActor().equals(plugin.lastCastedOnActorToTransmogHitSplashOn)) {
			if (plugin.lastCastedOnActorToTransmogHitSplashOn.getSpotAnimFrame() == 1 && lasttargetspotanimframe != 1) {
//				currentProjectile.hitGfxStart.put() = client.getGameCycle() - currentProjectile.spellStart;
			}
			lasttargetspotanimframe = plugin.lastCastedOnActorToTransmogHitSplashOn.getSpotAnimFrame();
		}
	}
